...
async function run() {
    try {
        ...
        console.log('--- Benchmarking Optimized Part 2 ---');

        console.time('Optimized Part 2 Time');
        const query = `
            SELECT 
                ...
                (SELECT CASE WHEN COUNT(*) > 0 THEN 'S' ELSE 'N' END
                 ...
                 AND (
                     (COALESCE(M1.CODFILIAL, M2.CODFILIAL) = '20' AND I.CODFILIALRETIRA IN ('20', '6'))
                     OR
                     (COALESCE(M1.CODFILIAL, M2.CODFILIAL) <> '20' AND I.CODFILIALRETIRA = COALESCE(M1.CODFILIAL, M2.CODFILIAL))
                 )
                ) AS TEM_PENDENCIA,
                ...
                (SELECT NVL(SUM(
                 ...
                 AND (
                     (COALESCE(M1.CODFILIAL, M2.CODFILIAL) = '20' AND E.CODFILIAL IN ('20', '6'))
                     OR
                     (COALESCE(M1.CODFILIAL, M2.CODFILIAL) <> '20' AND E.CODFILIAL = COALESCE(M1.CODFILIAL, M2.CODFILIAL))
                 )
                ) AS QTDISP
        ...
        console.timeEnd('Optimized Part 2 Time');
        console.log(`Optimized Part 2 returned ${result.rows.length} rows.`);
