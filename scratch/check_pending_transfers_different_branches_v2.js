...
async function main() {
    ...
    try {
        console.log('=== BUSCANDO ITENS DE TRANSFERÊNCIA EM TRÂNSITO (CODCLI = DESTINO) ===');
        const query = `
        SELECT 
            T.NUMNOTA,
            T.NUMTRANSVENDA,
        ...
        WHERE T.DTCANCEL IS NULL
          ...
          -- Excluir transferências internas dentro da mesma região (DF 20/6 <-> 20/6)
          AND NOT (
        ...
        ORDER BY T.DTSAIDA DESC, T.NUMNOTA DESC, M.CODPROD ASC
        `;

        const res = await conn.execute(query, [], opt);
        console.log(`\nEncontrados ${res.rows.length} itens de transferência inter-região em trânsito.`);
