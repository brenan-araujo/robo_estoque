...
const C=93, FIL='20', DI='22/06/2026', DF='26/06/2026';
const PERIODO_LABEL = 'Semana 22/06 a 26/06/2026 (seg–sex)';
...
(async()=>{
  ...
  const D={c:C,fil:FIL,di:DI,df:DF};
  ...
  const vb = await one(`SELECT NVL(SUM(VLTOTAL),0) BRUTO FROM PCNFSAID WHERE CODUSUR=:c AND CODFILIAL=:fil AND DTCANCEL IS NULL AND CONDVENDA IN (1,7,9,14)
       AND DTSAIDA>=TO_DATE(:di,'DD/MM/YYYY') AND DTSAIDA<TO_DATE(:df,'DD/MM/YYYY')+1`, D);
  const dv = await one(`SELECT NVL(SUM(QT*PUNIT),0) DEVOL FROM PCMOV WHERE CODUSUR=:c AND CODFILIAL=:fil AND CODOPER='ED'
       AND DTMOV>=TO_DATE(:di,'DD/MM/YYYY') AND DTMOV<TO_DATE(:df,'DD/MM/YYYY')+1`, D);
  const liq = Number(vb.BRUTO)-Number(dv.DEVOL);
  const pos = await one(`SELECT COUNT(DISTINCT CODCLI) N FROM PCPEDC WHERE CODUSUR=:c AND CODFILIAL=:fil AND DTCANCEL IS NULL AND CONDVENDA IN (1,7,9,14)
       AND DATA BETWEEN TO_DATE(:di,'DD/MM/YYYY') AND TO_DATE(:df,'DD/MM/YYYY')`, D);
  const nov = await one(`SELECT COUNT(DISTINCT cl.codcli) N FROM pcclient cl
     JOIN pcmov pm ON pm.codcli=cl.codcli JOIN pcpedc pp ON pp.numped=pm.numped AND pp.dtcancel IS NULL AND pp.condvenda IN (1,7,9,14) AND pp.vltotal>=120
     WHERE pm.codoper='S' AND pm.codfilial=:fil AND pm.codusur=:c AND pm.dtmov BETWEEN TO_DATE(:di,'DD/MM/YYYY') AND TO_DATE(:df,'DD/MM/YYYY')
       AND NOT EXISTS (SELECT 1 FROM pcmov pm2 WHERE pm2.codcli=cl.codcli AND pm2.codoper='S' AND pm2.dtmov<TO_DATE(:di,'DD/MM/YYYY'))`, D);
  const res = await one(`SELECT COUNT(DISTINCT CODCLI) N FROM PCPEDC PE
     WHERE PE.CODUSUR=:c AND PE.CODFILIAL=:fil AND PE.DTCANCEL IS NULL AND PE.CONDVENDA IN (1,7,9,14) AND PE.POSICAO='F' AND PE.VLTOTAL>=120
       ...
       AND EXISTS (SELECT 1 FROM PCPEDC P3 WHERE P3.CODCLI=PE.CODCLI AND P3.DTCANCEL IS NULL AND P3.CONDVENDA IN (1,7,9,14) AND P3.POSICAO='F'
            AND P3.DATA < ADD_MONTHS(TO_DATE(:di,'DD/MM/YYYY'),-6))`, D);
  const mix = await one(`SELECT COUNT(DISTINCT CODPROD) N FROM PCMOV WHERE CODUSUR=:c AND CODFILIAL=:fil AND CODOPER='S'
       AND DTMOV BETWEEN TO_DATE(:di,'DD/MM/YYYY') AND TO_DATE(:df,'DD/MM/YYYY')`, D);
  await conn.close(); await database.close();

  const d = {
    metaVenda:Number(meta.MV), liquida:liq,
...
• Meta: ${d.metaMix} | Realizado: ${d.mixReal} | Dif: ${sgn(d.mixReal-d.metaMix)} | Resultado: *${f1(pct(d.mixReal,d.metaMix))}%*`;
  ...
  if (process.argv[2]==='send') {
    ...
    const req=http.request({host:'localhost',port:3001,path:'/api/whatsapp/send-message',method:'POST',
      ...
      r=>{let b='';r.on('data',d=>b+=d);r.on('end',()=>console.log(`HTTP ${r.statusCode}: ${b}`));});
    req.on('error',e=>console.error('Erro:',e.message)); req.write(payload); req.end();
  }
})();
